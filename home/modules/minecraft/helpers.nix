  ...
  buildMinecraftClient = { version, username, }:
    ...
    in pkgs.runCommand "minecraft-${version}" {
      ...
      ${lib.concatMapStringsSep "\n" (library: ''
        unzip ${
          pkgs.fetchurl {
            inherit (library.downloads.classifiers.${library.natives.linux})
              url sha1;
          }
      ...
      '') (builtins.filter (x: (x.downloads ? "classifiers")) librariesData)}
        ...
        ${
          lib.concatStringsSep "\n" (builtins.attrValues
            (lib.flip lib.mapAttrs assetsIndex.objects (name: a:
              let
                asset = pkgs.fetchurl {
                  sha1 = a.hash;
                ...
                outPath = if versionInfo.assets == "legacy" then
                  "$out/assets/virtual/legacy/${name}"
                else
                  "$out/assets/objects/${hashTwo}";
              in ''
            ...
            ln -s ${
              builtins.toFile "assets.json" (builtins.toJSON assetsIndex)
            } \
